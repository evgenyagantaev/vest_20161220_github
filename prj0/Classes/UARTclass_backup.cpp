...
class clsUART //UART5 class /w DMA transmit, interrupt recieve
{
...
public://***********************************************************************
  ...
  void initUart()
  {
    /* --------------------------- System Clocks Configuration -----------------*/
     ...
     GPIO_PinAFConfig(GPIOB, GPIO_PinSource6, GPIO_AF_USART1); //usart1 rx
     ...
     GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_7; // PB.6-7 USART1 RX,TX 
 ...
 //  USART_ITConfig(USART3, USART_IT_RXNE, DISABLE);
  ...
  void sendMessage(char *outData)
  {
     ...
     else // no room in buffer (it's an emergency situation)
     {
        ...
        while(1)
        {
           sprintf(emergencyMessage, "uart out strings buffer overflow!!!\r\n");
           transmitMessage(emergencyMessage);
     ...
     if(outStringsBuffer[0] != 0) // there is string to transmit yet
     {
        if(uartOutputInProcess == 0) // previous transmission is finished
        {
          //if(!common.commandMode) sdio.writeStringToBuffer(outStringsBuffer[0]);
           ...
           DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&USART1->DR; //usart 3
  ...
  // function transmits one byte through usart3 without dma
